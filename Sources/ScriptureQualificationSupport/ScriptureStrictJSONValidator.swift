...
enum ScriptureStrictJSONValidator {
    static func validate(_ data: Data) throws {
        var parser = ScriptureStrictJSONParser(bytes: Array(data))
        try parser.parseDocument()
    }
}

private struct ScriptureStrictJSONParser {
    let bytes: [UInt8]
    var index = 0
    ...
    mutating func parseDocument() throws {
        ...
        try parseValue(path: "$")
        skipWhitespace()
        guard index == bytes.count else { throw ScriptureQualificationError.malformedManifest }
    }

    private mutating func parseValue(path: String) throws {
        guard let byte = current else { throw ScriptureQualificationError.malformedManifest }
        switch byte {
    ...
    private mutating func parseObject(path: String) throws {
        ...
        while true {
            ...
            guard fields.insert(field).inserted else {
                throw ScriptureQualificationError.duplicateJSONField(path: path, field: field)
            }
            skipWhitespace()
            try require(0x3A)
            skipWhitespace()
    ...
    private mutating func parseString() throws -> String {
        guard current == 0x22 else { throw ScriptureQualificationError.malformedManifest }
        let start = index
        index += 1
        while let byte = current {
            ...
            guard byte >= 0x20 else { throw ScriptureQualificationError.malformedManifest }
            index += byte == 0x5C ? 2 : 1
            guard index <= bytes.count else { throw ScriptureQualificationError.malformedManifest }
        }
    ...
    private mutating func consumeNumberToken() throws {
        ...
        guard index > start else { throw ScriptureQualificationError.malformedManifest }
    }

    private mutating func require(_ byte: UInt8) throws {
        guard consume(byte) else { throw ScriptureQualificationError.malformedManifest }
    }
