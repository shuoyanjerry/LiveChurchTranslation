...
mutable state in separate actors.
...
the same HttpOnly grant-cookie extractor. URLs with a query are rejected, preventing token leakage.
...
handshake rules, viewer mutation denial, secure response headers, and a real ephemeral localhost listener
serving an asset plus snapshot/live WebSocket data.
