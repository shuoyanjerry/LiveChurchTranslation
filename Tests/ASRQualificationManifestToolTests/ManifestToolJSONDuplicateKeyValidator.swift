...
enum ManifestToolJSONDuplicateKeyValidator {
    static func validate(_ data: Data, source: String) throws {
        var parser = ManifestToolJSONParser(bytes: Array(data), source: source)
        try parser.parseDocument()
    }
}

private struct ManifestToolJSONParser {
    let bytes: [UInt8]
    let source: String
    var index = 0
    ...
    private mutating func parseObject(path: String) throws {
        ...
        while true {
            ...
            guard fields.insert(field).inserted else {
                throw ManifestToolError.duplicateJSONField(
                    source: source,
                    path: path,
                    field: field
                )
    ...
    private var malformed: ManifestToolError {
