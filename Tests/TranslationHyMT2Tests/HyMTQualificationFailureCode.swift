...
enum HyMTQualificationFailureCode {
    static func make(_ error: any Error) -> String {
        ...
        let digest = TranslationQualificationSHA256.hash(data: Data(typeName.utf8))
...
private enum StrictValidationCategory: String, CaseIterable {
    ...
    static func classify(_ reason: String) -> StrictValidationCategory {
        if let exact = exactReasons[reason] { return exact }
        if knownPrefixes.contains(where: reason.hasPrefix) {
            return .pronounProtocol
        }
    ...
    private static let exactReasons: [String: StrictValidationCategory] = [
        "empty output": .empty,
