...
enum FunQualificationHostEnvironment {
    static func detect(
        ...
        ASRQualificationEnvironmentV3(
            ...
            backgroundLoadNote: environment["FUNASR_ASR_BACKGROUND_LOAD_NOTE"]
    ...
    private static func command(
        ...
        guard process.terminationStatus == 0 else {
            throw FunQualificationEnvironmentError.commandFailed
        }
        let data = output.fileHandleForReading.readDataToEndOfFile()
        guard let value = String(bytes: data, encoding: .utf8) else {
            throw FunQualificationEnvironmentError.invalidOutput
        }
        return value.trimmingCharacters(in: .whitespacesAndNewlines)
    }
}

enum FunQualificationEnvironmentError: Error, Equatable {
    case commandFailed
    case invalidOutput
}
